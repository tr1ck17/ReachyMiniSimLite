"""
Reachy Mini Solar System Program (Simulation + Offline Voice)

Combines:
- Simulation movement via the Reachy Mini Dashboard (localhost)
    ...
    def __init__(self, sample_rate: int = 16000, record_seconds: float = 5.0) -> None:
        ...
        if not os.path.isdir(VOSK_MODEL_PATH):
            raise RuntimeError(
                ...
                "Run setup_sim_voice.ps1 or download from https://alphacephei.com/vosk/models"
            )
...
class ReachySimVoiceAdapter(ReachyAdapter):
    """
    Reachy Mini SDK adapter for the Dashboard Simulation
...
def parse_menu_choice(raw: str, option_keywords: Dict[str, str]) -> Optional[str]:
    ...
    Args:
        ...
        option_keywords: Mapping of keyword phrase -> choice number.
            e.g. {"full lesson": "1", "single level": "2", ...}
...
def run() -> None:
    print("""
    =============================================
      Reachy Mini  --  Solar System Courseware
      Simulation + Offline Voice (TTS & STT)
    =============================================
    
    Make sure the Reachy Mini Dashboard is open
    with Simulation mode selected and the robot
    awake before continuing.
    """)

    voice = build_voice()

    # Connect to the Dashboard simulation on localhost
    with ReachyMini(connection_mode="localhost_only") as mini:
        adapter = ReachySimVoiceAdapter(mini, voice)
        adapter.say(
